...
class AccessoryRegistry extends ChangeNotifier {
  ...
  Future<void> loadAccessories() async {
    loading = true;
    String? serialized = await AppStorage.read(accessoryStorageKey);
    if (serialized != null) {
      try {
        final accessoryJson = jsonDecode(serialized) as List<dynamic>;
        _accessories = accessoryJson
            .map((val) => Accessory.fromJson(Map<String, dynamic>.from(val)))
            .toList();
      } catch (_) {
        _accessories = [];
      }
  ...
  Future<void> _storeAccessories() async {
    final jsonList =
        _accessories.map((accessory) => accessory.toJson()).toList();
    await AppStorage.write(accessoryStorageKey, jsonEncode(jsonList));
  }
